...
//            }
            ...
            tmp_TextView.setOnClickListener(new TextView.OnClickListener(){
                ...
                public void onClick(View v) {
                    if (tab_question.getAnswer_choose() == 999) {
                        ...
                        //set câu đã chọn
                        MainActivity.currArrayListQuesstionGroupClass.get(MainActivity.currQuesstionGroupClass).getQuestionClassArrayList().get(pos).setAnswer_choose(finalI);
                        true_view.setBackground(getResources().getDrawable(R.drawable.my_border_answer_true));
            ...
            textViews.add(tmp_TextView);

        }
        for (TextView textView: textViews)
            ln_answer.addView(textView);

        if (tab_question.getAnswer_choose() != 999) {
            if (tab_question.isChoose_true()){
                textViews.get(tab_question.getAnswer_choose()).setBackground(getResources().getDrawable(R.drawable.my_border_answer_choose_true));
                MainActivity.tabLayout.getTabAt(pos).setIcon(R.drawable.star_ok);
            }else{
                textViews.get(tab_question.getAnswer_choose()).setBackground(getResources().getDrawable(R.drawable.my_border_answer_choose));
                textViews.get(tab_question.getAnswer_true()).setBackground(getResources().getDrawable(R.drawable.my_border_answer_true));
                MainActivity.tabLayout.getTabAt(pos).setIcon(R.drawable.star_ng);
            }
        }
        return result;
